...
import { TodoUpdateRepository } from '../todo.update.repository';
import { TodoUpdateOutboundPortOutputDto } from '../../outbound-port/todo.update.outbound-port';
import { TodoDeleteRepository } from '../todo.delete.repository';
import { TodoDeleteOutboundPortOutputDto } from '../../outbound-port/todo.delete.outbound-port';

dotenv.config();

describe('TodoDeleteRepository Spec', () => {
  let todoDeleteRepository: TodoDeleteRepository;
  ...
  beforeAll(async () => {
    ...
    em = orm.em;
    todoDeleteRepository = new TodoDeleteRepository(em);
  ...
  test('투두생성 : 투두를 삭제합니다.', async () => {
    const params = {
      todoId: '2',
      userId: '1',
    };

    const result: TodoDeleteOutboundPortOutputDto =
      await todoDeleteRepository.execute(params);
